///////////////////////////////////////////////////////////////////////////////////////          
//Example of usage divine proportion with sequence of golden rectangles
/////////////////////////////////////////////////////////////////////////////////////// 
...
namespace GoldenRectanglesRegression
{
	struct Point2
	{
	...
	void DrawRectangleAt(Canvas& cvs, const Point2& At, float width, float height)
	{
		cvs.MoveTo(At);
	...
	void DrawRectanglesRegression(Canvas& cvs, std::pair<float, float> widthHeight)
	{
		bool XTime = true;

		while(!IsNearlyEqual(widthHeight.first, widthHeight.second, 0.1f))
		{
			const auto moveLength = std::min(widthHeight.first, widthHeight.second);

			cvs.Forward(moveLength, false);
			cvs.Turn(90);
			cvs.Forward(moveLength);	

			if(XTime)
			{
				widthHeight = {std::abs(widthHeight.first - moveLength), widthHeight.second};
			}
			else
			{
				widthHeight = {widthHeight.first, std::abs(widthHeight.second - moveLength)};
			}
	...
	std::pair<float, float> GetNearGoldenRectangle(float width, float height)
	{
		const auto maxHeight = std::max(width, height) / PHI;
		return {maxHeight * PHI, maxHeight};
	}
	...
	void Display()
	{
		...
		const auto widthHeight = GetNearGoldenRectangle(600, 400);
		const auto StartPoint = Point2(cvs.GetScreenWidth() / 2 - widthHeight.first / 2, cvs.GetScreenHeight() / 2 - widthHeight.second / 2);

		DrawRectangleAt(cvs, StartPoint, widthHeight.first, widthHeight.second);
		DrawRectanglesRegression(cvs, widthHeight);
	}
	
	void printInteraction()
	{
		std::cout << "This program shows sequence of golden rectangles in specified golden rectangle\n";
	...
	int main(int argc, char** argv)
	{
		...
		auto& cvs = Canvas::Get();	
		cvs.Init(640, 480, "Golden Rectangle try");
